...
public class FeedCron
    ...
    /**
     * Will be called every minute by the Cron Job on the Google app engine {@inheritDoc}
     */
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException,
        ...
        for (FeedConfiguration config : configs) {
            Calendar cal = config.getNextUpdate();
            if (Math.round(cal.getTimeInMillis() / 1000) == Math.round(currentTime
                .getTimeInMillis() / 1000)) {
                activeFeedURLs.add(config.getFeedUrl());
                config.setPreviousUpdate(currentTime);
                currentTime.add(Calendar.MINUTE, config.getFeedUpdateInterval());
                config.setNextUpdate(currentTime);
            }
        }
